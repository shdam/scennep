...
#'
#' @return The input Seurat object with an additional assay named "scennep", which contains
...
scennep <- function(
  ...
  # Return seurat
  if (class(obj)[1] != "Seurat" & as != "seurat") {
    return_seurat <- FALSE
  }
  ...
  if (class(obj)[1] %in% c("matrix", "dgCMatrix")) {
    if (as == "seurat") {
      obj <- Seurat::CreateSeuratObject(counts = Seurat::CreateAssayObject(counts = obj, key = "RNA_"), assay = "RNA")
  ...
  # More memory efficient to rm Seurat object
  if (!return_seurat) {
    rm(obj)
    gc()
  }
...
build_snn_seurat <- function(seu_obj, nn_count, npcs) {
  ...
  if (!any(detect_snn)) {
    ...
    seu_obj <- Seurat::FindNeighbors(
      ...
      # k.param = nn_count,
      compute.SNN = TRUE)
    slot <- SeuratObject::Graphs(seu_obj)[grepl("_snn", SeuratObject::Graphs(seu_obj))]
  } else {
...
build_snn_sce <- function(sce, nn_count, assay, npcs) {
  ...
  snn_graph <- igraph::as_adjacency_matrix(snn_graph)
  return(snn_graph)
}
