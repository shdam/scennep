...
#'   cell has fewer than this number of neighbors, all are used. Default is 20.
...
#' @param pc_explained The fraction of variance explained cutoff for selecting npcs. Overwritten if `npcs` are provided.
...
#' @param features Subset to features of interest (overwritten if `markers` is provided).
...
build_snn_seurat <- function(seu_obj, nn_count, npcs) {
    ...
    return(list("obj" = seu_obj, "graph" = snn_graph))
}
...
build_snn_sce <- function(sce, nn_count, npcs) {
    ...
    return(list("obj" = sce, "graph" = snn_graph))
}
